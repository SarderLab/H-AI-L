...
def get_choppable_regions(wsi,index_x, index_y, boxSize, object_size=150, se=disk(15)):
    if wsi.split('.')[-1] != 'tif':
        slide=getWsi(wsi)
        ...
        binary_closed=binary_closing(binary,se)
        binary_closed=binary_fill_holes(binary_closed)
        binary_closed=remove_small_objects(binary_closed,object_size)
        ...
        for idxy,yi in enumerate(index_y):
            for idxx,xj in enumerate(index_x):
                ...
                if np.sum(binary_closed[yStart:yStop,xStart:xStop])>0:
